/**
 * Default Cytoscape Layout Configurations
 *
 * Migrated from packages/utils/src/atlas/layout-configs.ts
  ...
  switch (type) {
    ...
    case "atlas":
      return {
        ...
        animate: "end", // This will be overridden by the hook for performance
        animationDuration: 1000,
        animationEasing: "ease-out",
        nodeRepulsion: 3000,
        idealEdgeLength: 120,
...
export const defaultLayouts = {
  breadthfirst: getLayoutConfig("breadthfirst"),
  force: getLayoutConfig("force"),
  atlas: getLayoutConfig("atlas"),
};

/**
 * Create custom layout configuration with overrides
 */
export function createLayoutConfig(
  type: LayoutType,
  overrides: Partial<CytoscapeLayoutConfig> = {}
): CytoscapeLayoutConfig {
  const baseConfig = getLayoutConfig(type);
