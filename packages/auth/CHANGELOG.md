...
- 8a48e27: Add tier-based document size limits to UserTierLimits interface

  Added `maxDocumentSize` and `documentHistoryRetentionDays` fields to support Yjs document size management with automatic history pruning. Limits vary by tier from 10MB (Free) to 500MB (Enterprise) with retention periods from 0 days (Free, snapshot-only) to unlimited (Enterprise).
