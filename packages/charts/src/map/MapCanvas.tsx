...
const log = DEBUG ? console.log.bind(console, "[MapCanvas]") : () => {};
...
export interface MapCanvasProps {
  ...
  entities: MapEntity[];
  /**
   * @deprecated Not currently rendered. Accepted for API compatibility with graph-based components.
   * May be used in future for rendering relationship lines on the map.
   */
...
export function MapCanvas({
  ...
  // Refs for stable event handlers (prevents stale closures)
  const onMapClickRef = useRef(onMapClick);
  const onBoundsChangeRef = useRef(onBoundsChange);
  const onEntityClickRef = useRef(onEntityClick);

  // Store Leaflet module reference to avoid repeated dynamic imports
  // Using any type here because Leaflet's ESM default export is complex
  ...
  }, [onMapClick, onBoundsChange, onEntityClick]);
  ...
  useEffect(() => {
    ...
    import("leaflet")
      .then((LeafletModule) => {
        leafletRef.current = LeafletModule.default; // Store reference for reuse
        ...
        if (containerWithLeaflet._leaflet_id) {
          log("Clearing existing Leaflet instance");
      ...
      .catch((err) => {
        if (DEBUG) console.error("[MapCanvas] Failed to load Leaflet:", err);
    ...
    return () => {
      if (map) {
        ...
        try {
          ...
          if (DEBUG) console.error("[MapCanvas] Cleanup error:", err);
        }
      }
    };
  }, [map]);
  ...
  useEffect(() => {
    if (!map) return;
    if (!leafletRef.current) return; // Wait for Leaflet to be loaded

    log(`Adding ${entities.length} markers`);

    // Use stored Leaflet reference (already imported during initialization)
    ...
    entities.forEach((entity) => {
      ...
      marker.on("click", () => onEntityClickRef.current?.(entity));

      markersRef.current.push(marker);
    });

    log(`Added ${markersRef.current.length} markers`);
  }, [map, entities]);
